...
class TestCreateMonitor:
    ...
    async def test_create_monitor_success(self, client: AsyncClient, auth_headers):
        ...
        with patch("app.api.v1.monitor.check_rate_limit_full", new_callable=AsyncMock) as mock_rl, \
            ...
            mock_rl.return_value = MagicMock(allowed=True, limit=30, remaining=29, reset=60)
            mock_task.delay = MagicMock()

            resp = await client.post("/v1/monitors", json={
    ...
    @pytest.mark.asyncio
    async def test_create_monitor_invalid_interval(self, client: AsyncClient, auth_headers):
    ...
    @pytest.mark.asyncio
    async def test_create_monitor_invalid_notify_on(self, client: AsyncClient, auth_headers):
...
class TestListMonitors:
    ...
    async def test_list_monitors_after_create(self, client: AsyncClient, auth_headers):
        ...
        with patch("app.api.v1.monitor.check_rate_limit_full", new_callable=AsyncMock) as mock_rl, \
            ...
            await client.post("/v1/monitors", json={
                "name": "My Monitor",
                "url": "https://example.com",
                "check_interval_minutes": 60,
                "notify_on": "any_change",
...
class TestUpdateMonitor:
    ...
    async def test_update_monitor(self, client: AsyncClient, auth_headers):
        ...
        with patch("app.api.v1.monitor.check_rate_limit_full", new_callable=AsyncMock) as mock_rl, \
            ...
            create_resp = await client.post("/v1/monitors", json={
                "name": "Original Name",
                "url": "https://example.com",
                "check_interval_minutes": 60,
                "notify_on": "any_change",
        ...
        resp = await client.patch(f"/v1/monitors/{monitor_id}", json={
            "name": "Updated Name",
        }, headers=auth_headers)
...
class TestTriggerCheck:
    ...
    async def test_trigger_check(self, client: AsyncClient, auth_headers):
        ...
        with patch("app.api.v1.monitor.check_single_monitor_task") as mock_task:
            ...
            resp = await client.post(f"/v1/monitors/{monitor_id}/check", headers=auth_headers)
            assert resp.status_code == 200
            assert resp.json()["success"] is True
...
class TestMonitorHistory:
    ...
    async def test_history_empty(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.get(f"/v1/monitors/{monitor_id}/history", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["success"] is True
        assert data["checks"] == []
    ...
    async def test_history_not_found(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.get(f"/v1/monitors/{uuid.uuid4()}/history", headers=auth_headers)
        assert resp.status_code == 404
