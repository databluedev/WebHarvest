...
class TestSaveLLMKey:
    ...
    async def test_save_llm_key_success(self, client: AsyncClient, auth_headers):
        ...
        with (
            patch("app.api.v1.settings.encrypt_value", return_value="encrypted_abc"),
            patch(
                "app.api.v1.settings.decrypt_value",
                return_value="sk-test1234567890abcdef",
            ),
        ):
    ...
    async def test_save_llm_key_upsert(self, client: AsyncClient, auth_headers):
        ...
        with (
            ...
            patch(
                "app.api.v1.settings.decrypt_value",
                return_value="sk-first-key-12345678",
            ),
        ):
        ...
        with (
            patch("app.api.v1.settings.encrypt_value", return_value="encrypted_def"),
            patch(
                "app.api.v1.settings.decrypt_value",
                return_value="sk-second-key-1234567",
            ),
        ):
    ...
    @pytest.mark.asyncio
    async def test_save_llm_key_missing_provider(
...
class TestListLLMKeys:
    ...
    async def test_list_llm_keys_after_create(self, client: AsyncClient, auth_headers):
        ...
        with patch(
            "app.api.v1.settings.decrypt_value", return_value="sk-test1234567890abcdef"
        ):
            resp = await client.get("/v1/settings/llm-keys", headers=auth_headers)
...
class TestDeleteLLMKey:
    ...
    async def test_delete_llm_key(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.delete(
            f"/v1/settings/llm-keys/{key_id}", headers=auth_headers
        )
        assert resp.status_code == 200
    ...
    async def test_delete_llm_key_not_found(self, client: AsyncClient, auth_headers):
        ...
        resp = await client.delete(
            f"/v1/settings/llm-keys/{uuid.uuid4()}", headers=auth_headers
        )
